...
- **T2 — Capture QR server-side.** `pawnoteServerAdapter.connectWithQrPayload({ qr, pin, deviceUuid })` via `pawnote.loginQrCode` → session + metadata (`instanceUrl`, `username`, `kind`, `resources`). Service `pronoteConnectService.connectQr(userId, payload)` → `upsertCredentials` (jeton chiffré, `establishmentUrl` normalisée) → renvoie `credentialId` + enfants découverts. Route `POST /api/pronote/connect/qr` (guard auth ; ne logge jamais jeton/identifiants, seulement compteurs/host/kind). **Dérisque le token rotatif** (`loginQrCode → loginToken`).
