...
- **Profil-enfant = `user role=student`** (déjà le cas via `createChild`), classe/établissement re-synchronisés depuis Pronote. **Login élève autonome activé** (décision 2026-06-18) : le parent définit `username`+mot de passe à l'activation ; l'enfant se connecte seul (login par username, Better Auth déjà en place) et accède à son espace (chat, révisions, ses données Pronote via l'accès `self` déjà autorisé sur les routes). Le parent reste lié (consultation + impersonation).
...
- **Dépendance reverse-eng + écosystème 100 % GPL** (vérifié gh) : Pronote peut casser pawnote à une MAJ → **version-lock `1.6.2`** (npm up ; pawnote **maintenu** — Papillon `dev` en dépend en prod, rewrite 2.0 ; code source **déménagé sur Radicle**, d'où le 404 GitHub), monitoring. **Toutes les libs Pronote actives sont GPL** : pawnote (GPL-3.0) ; **Blocksnote** (futur wrapper unifié de l'équipe Papillon / org BlocksHub, **GPL-3.0**, **v0.0.1** immature). La seule en MIT (**pronotepy**, Python) est en **mode maintenance** (features gelées). → **pas d'échappatoire MIT crédible** : le « serveur-only » n'est pas un choix esthétique mais une **nécessité de conformité**. Pérennité long terme : **voie officielle Docaposte** (B2B, par établissement, payant).
