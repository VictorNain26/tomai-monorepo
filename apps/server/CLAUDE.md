...
- **Pronote** : dual-mode — device-first sur mobile (Pawnote, student auth) ET server-side provider (`PawnoteServerAdapter`, `loginToken`) pour les lectures parent/web ; le serveur déchiffre le token en mémoire pour ses propres appels. Credentials stockés chiffrés AES-256-GCM (PBKDF2 600K iterations, salt aléatoire par enregistrement).
