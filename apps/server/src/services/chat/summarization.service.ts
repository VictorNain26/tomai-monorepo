 ...
 *  - Mistral Small 4 non-streaming, temperature 0.3 pour cohérence
 *  - Reasoning effort `none` : on veut un résumé descriptif, pas un raisonnement
 */
...
export const SUMMARIZATION_PROMPT_VERSION = '2026-05-06';
...
class SummarizationService {
  ...
  private async generateSummary(
    ...
    let prompt: string;
    if (previousSummary) {
      prompt = INCREMENTAL_PROMPT.replace('{previousSummary}', previousSummary).replace(
        '{newMessages}',
        messagesText,
      );
    } else {
    ...
    const response = await withTimeout(
      this.client.chat.complete({
        ...
        temperature: 0.3,
        maxTokens: 2048,
        // Descriptive recap of a conversation: no chain-of-thought needed.
